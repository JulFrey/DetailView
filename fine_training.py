...
@author: Julian
@description: retrain the model with a subset of species
"""
...
# select species to retrain the model
species = [0,1,2,4,5,67,9,10,12,13,14,21,24,25,27,28,29,30,31]
train_metadata = train_metadata[train_metadata["species_id"].isin(species)]
# write new csv as train_labels_subset.csv
train_metadata.to_csv(path_las + "/train_labels_subset.csv", index = False)
vali_metadata = pd.read_csv(path_csv_vali)
vali_metadata = vali_metadata[vali_metadata["species_id"].isin(species)]
vali_metadata.to_csv(path_las + "/vali_labels_subset.csv", index = False)

path_csv_train = path_las + "/train_labels_subset.csv"
path_csv_vali = path_las + "/vali_labels_subset.csv"
...
# freeze the model besides the classifier
...
for epoch in range(num_epochs):
    ...
    if avg_v_loss < best_v_loss:
        ...
        model_path = "model_ft_{}_{}".format(timestamp, epoch + 1)
...
df.to_csv('loss_ft_' + timestamp + '.csv', index=False)
