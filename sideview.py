...
def points_to_images(path_las, res_im = 256, num_side = 4, plot = False):
    ...
    # prepare view array
    views = np.zeros((num_side + 1, res_im, res_im))
...
# creating topview
def topview(points, res_im, plot = False):
    ...
    for point in points:
        ...
        if point[2] > top_image[x_pos, y_pos]:
            top_image[x_pos, y_pos] = point[2]
...
def sideview(points, res_im, plot = False):
    ...
    for point in points:
        ...
        # update the corresponding pixel in the depth image with the z coordinate
        if point[1] > side_image[x_pos, z_pos]:
            side_image[x_pos, z_pos] = point[1]
...
# example 
views = points_to_images(r"D:\Baumartenklassifizierung\data\raw\03498.las", plot = True)
